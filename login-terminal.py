...
import urllib,sys,threading,webbrowser
...
class Proxy:
	...
	def __init__(self, username, password, proxy_cat):
		...
		self.urlopener = request.build_opener(request.ProxyHandler({'auto_proxy':self.auto_proxy}))
	...
	def is_connected(self):
		...
		try:
			response = str(urllib.urlopen(Proxy.google, proxies=proxies).read())
		except:
	...
	def get_session_id(self):
		try:
			response = str(self.open_page(self.proxy_page_address))
		except:
			return None
		check_token = 'sessionid" type="hidden" value="'
		token_index=response.find(check_token) + len(check_token)
		sessionid=""
		for i in range(16):
	...
	def login(self):
		self.new_session_id()
		response = str(self.submitform(self.loginform))
	...
	def logout(self):
		self.loggedout = True
		response = str(self.submitform(self.logout_form))
	...
	def refresh(self):
		response = str(self.submitform(self.loggedin_form))
	...
	def submitform(self, form):
		return self.urlopener.open(request.Request(self.proxy_page_address, parse.urlencode(form).encode('utf-8'))).read()
